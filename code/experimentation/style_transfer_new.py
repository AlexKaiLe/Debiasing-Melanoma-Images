...
import os
from cnn_train_2 import Model, test
...
def visualize_image(image_tensor, label):
	...
	"""
	image = np.array(image_tensor).reshape((224,224,3))
...
def visualize_loss(loss):
	...
	plt.plot(x, loss)
	plt.savefig('../figures/StyleTransfer_LossperIteration_2.png')
	plt.show()
	...
	plt.savefig('../figures/StyleTransfer_LossperIteration_after200_2')
	plt.show()
...
def main():
	...
	model_init(tf.zeros((1,224,224,3)), dense=True)
	model(tf.zeros((1,224,224,3)), dense=False) 
	train_model(tf.zeros((1,224,224,3)), dense=False) 
	model_init.load_weights('../checkpoints/alex.weights.e019-acc0.4322.h5') # load weights into old model
	...
	train_dataset, test_dataset = get_data('../ISIC_data/Train/', '../ISIC_data/Test/', 
											...
											image_sz=(224,224)) # try changing to (600,450) on GPU
	...
	##### STYLE PREPROCESS #####
	style_image = tf.keras.preprocessing.image.load_img('../preprocessed_images/img13.jpg', target_size=(224, 224))
	style_image = tf.Variable(tf.reshape(tf.keras.preprocessing.image.img_to_array(style_image), (1, 224, 224, 3))/255., trainable=False)

	##### GENERATE STATIC IMAGE #####
	# generated_image = tf.Variable(tf.random.uniform((1,224,224,3), minval=0, maxval=1), name='Generated_Image', trainable=True)
	...
	visualize_image(styletransfer.image, 'initial_image_2') # visualize initial input
	...
	visualize_image(styletransfer.image, 'final_image_2') # visualize output
