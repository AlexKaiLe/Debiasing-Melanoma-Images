...
class Model(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.block2_pool = MaxPool2D(pool_size=(2, 2), name="block2_pool")
        # Block 3
        ...
        self.block3_pool = MaxPool2D(pool_size=(4, 4), name="block3_pool")

        # Dense Layers for Classification
        self.block3_dropout = Dropout(rate=0.3, name="block3_dropout")
        self.flatten = Flatten()
        self.dense1 = Dense(512, activation='relu', name='dense1')
        self.dense2 = Dense(9, activation='softmax', name='dense3')
    ...
    def call(self, inputs, dense=False, style=False, is_training=False, feature=False):
        ...
        # Dense Layers for Classification
        if dense:
            x = self.block3_dropout(x, training=is_training)
            x = self.flatten(x)
            x = self.dense1(x)
            x = self.dense2(x)
            return x
