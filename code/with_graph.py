...
def train(model, train_dataset):
	...
	for train_inputs, train_labels in train_dataset:
		...
		with tf.GradientTape() as tape:
			...
			loss_out += model.loss(probs, train_labels)
...
def test(model, test_dataset):
	...
	accuracy = model.accuracy(probs, test_labels).numpy()
	loss = model.loss(probs, test_labels)
	return accuracy, loss 

def visualize_accuracy(training_accuracy, validation_accuracy):
	x = np.arange(1, len(training_accuracy)+1)
	x2 = np.arange(1, len(validation_accuracy)+1)
...
def visualize_loss(loss_training, loss_validation):
	x = np.arange(1, len(loss_training)+1)
	x2 = np.arange(1, len(loss_validation)+1)
	plt.xlabel('i\'th Batch')
	plt.ylabel('Loss Value')
	plt.title('Loss per Batch')
	# plt.plot(x, loss)
	plt.plot(x, loss_training, label = "Training Loss")
...
def main():
	...
	for e in range(model.epochs):
		...
		train_accuracy, train_loss = train(model, train_dataset)
		loss_list_train.append(train_loss)
		accuracy_list_train.append(train_accuracy)
	
		test_accuracy, test_loss = test(model, test_dataset)
		loss_list_test.append(test_loss)
		accuracy_list_test.append(test_accuracy)
		print(f'Test Accuracy after epoch {e+1}: {test_accuracy*100}%')
		model.save_weights('../checkpoints/weights.h5')
		print('Weights Saved!')

	test_acc = test(model, test_dataset)
	visualize_loss(loss_list_train, loss_list_test)
	visualize_accuracy(accuracy_list_train, accuracy_list_test)
